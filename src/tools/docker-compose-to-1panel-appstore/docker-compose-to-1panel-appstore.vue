...
const expandAppDeclaration = ref(true); // 控制应用声明卡片表单展开状态
const expandDockerComposeEditor = ref(true); // 控制Docker Compose文件卡片表单展开状态
const expandAppParams = ref(true); // 控制应用参数配置卡片表单展开状态
const expandTagForm = ref(true); // 控制应用参数配置标签部分表单展开状态
          ...
          <n-card :title="`参数 ${index + 1}`" class="mb-4">
            ...
            <n-card class="mb-4">
              <template #header>
                <div 
                  class="expand-collapse-card-header"
                  @click="expandTagForm = !expandTagForm"
                >
                  ...
                  <n-button 
                    ...
                    size="small"
                    text
                    @click.stop="expandTagForm = !expandTagForm"
                  >
                    <template #icon>
                      <n-icon>
                        <svg v-if="expandTagForm" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                          <path d="m18 15-6-6-6 6"/>
                        </svg>
                        <svg v-else xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                          <path d="m6 9 6 6 6-6"/>
            ...
            <n-card class="mb-4">
              <template #header>
                <div 
                  class="expand-collapse-card-header"
                  @click="expandDescriptionForm = !expandDescriptionForm"
                >
                  ...
                  <n-button 
                    ...
                    size="small"
                    text
                    @click.stop="expandDescriptionForm = !expandDescriptionForm"
                  >
                    <template #icon>
                      <n-icon>
                        <svg v-if="expandDescriptionForm" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                          <path d="m18 15-6-6-6 6"/>
                        </svg>
                        <svg v-else xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                          <path d="m6 9 6 6 6-6"/>
                    ...
                    {{ expandDescriptionForm ? '折叠' : '展开' }}
                  </n-button>
                </div>
              </template>
              <div v-show="expandDescriptionForm">
                <n-grid cols="2" x-gap="12">
                  <n-gi>
